...
def test_spectral_axis_conversions():
    ...
    wcs_spec = Spectrum(flux=np.random.randn(49) * u.Jy, wcs=WCS(wcs_dict),
...
def test_create_with_uncertainty():
    ...
    with pytest.raises(ValueError):
        Spectrum(spectral_axis=wavelengths*u.um, flux=flux, uncertainty=uncertainty)
...
def test_repr():
    spec_with_wcs = Spectrum(spectral_axis=np.linspace(100, 1000, 10) * u.nm,
                               flux=np.random.random(10) * u.Jy)
    result = repr(spec_with_wcs)
    assert result.startswith('<Spectrum(flux=<Quantity [')
    assert 'spectral_axis=<SpectralAxis' in result

    spec_with_unc = Spectrum(spectral_axis=np.linspace(100, 1000, 10) * u.nm,
                               flux=np.random.random(10) * u.Jy,
...
def test_str():
    ...
    spec_with_uncertainty = Spectrum(spectral_axis=np.linspace(100, 1000, 10) * u.nm,
                                       flux=np.random.random(10) * u.Jy,
    ...
    spec_multi_flux = Spectrum(spectral_axis=np.linspace(100, 1000, 10) * u.nm,
