...
#
# The zenodo record contains the entire set of 12 _x1d files corresponding
# to this data set. We use just a few files for the current tests. Maybe
# more files will be needed when more functionality is added later on.
filename_storage = []
@remote_access([
...
def test_loaddata_miri_mrs(remote_data_path):
    filename_storage.append(remote_data_path)
...
def test_spectrum_list_directory_miri_mrs():
    ...
    # run. And also because temp directories with partial common paths
    # created in previous runs may still be hanging around. That precludes
    # the use of path.commonpath().
    with tempfile.TemporaryDirectory() as tmp_dir:
        for file_path in filename_storage:
